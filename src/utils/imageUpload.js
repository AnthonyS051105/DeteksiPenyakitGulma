...
export const uploadToImgBB = async (imageFile) => {
  ...
  console.log('Uploading to ImgBB with API key:', apiKey.substring(0, 8) + '...');
  ...
  try {
    ...
    if (error.message.includes('fetch')) {
      throw new Error('Tidak dapat terhubung ke ImgBB. Periksa koneksi internet Anda.');
    } else {
...
export const processWithLeafGuardAI = async (imageUrl) => {
  ...
  try {
    ...
    return {
      ...
      description: result.description || result.message || 'Analisis penyakit daun telah selesai. Sistem AI telah menganalisis gambar daun yang Anda upload.',
      recommendation: result.recommendation || result.treatment || 'Silakan konsultasikan dengan ahli pertanian untuk penanganan lebih lanjut. Lakukan pemantauan rutin pada tanaman.',
    ...
    if (error.name === 'TypeError' && error.message.includes('fetch')) {
      throw new Error('Tidak dapat terhubung ke AI service. Periksa koneksi internet atau coba lagi nanti.');
    } else if (error.message.includes('CORS')) {
      throw new Error('CORS error: AI service mungkin tidak mengizinkan akses dari domain ini.');
    } else {
...
export const processWithNeuraWeedAI = async (imageUrl) => {
  ...
  try {
    ...
    return {
      success: true,
      prediction: result.prediction || result.result || result.class || 'Tidak terdeteksi gulma',
      confidence: result.confidence || null,
      description: result.description || result.message || 'Analisis deteksi gulma telah selesai. Sistem AI telah menganalisis area sekitar tanaman.',
      recommendation: result.recommendation || result.treatment || 'Silakan lakukan pembersihan gulma jika diperlukan. Gunakan metode yang ramah lingkungan.',
      processed_image_url: result.processed_image_url || null,
      raw_result: result
    };
  } catch (error) {
...
export const checkAIServiceHealth = async () => {
  ...
  try {
    ...
    if (error.name === 'TimeoutError') {
      return { success: false, error: 'Service timeout - mungkin sedang cold start' };
    }
